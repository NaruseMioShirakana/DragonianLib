...
struct ggml_context* ggml_libsvc_allocate(struct ggml_init_params params) {
    ...
    *ctx = (struct ggml_context){
    ...
    if (!ctx->mem_buffer || (((uintptr_t)(ctx->mem_buffer)) % GGML_MEM_ALIGN) != 0)
    {
...
void ggml_libsvc_free(struct ggml_context* ctx) {
    if (ctx == NULL) return;
    if (ctx->mem_buffer_owned && ctx->mem_buffer)
        GGML_ALIGNED_FREE(ctx->mem_buffer);
    GGML_ALIGNED_FREE(ctx);
...
void ggml_libsvc_free_tensor(struct ggml_context* ctx)
{
    *ctx = (struct ggml_context){
        /*.mem_size           =*/ ctx->mem_size,
        /*.mem_buffer         =*/ ctx->mem_buffer,
        /*.mem_buffer_owned   =*/ ctx->mem_buffer_owned,
        /*.no_alloc           =*/ ctx->no_alloc,
        /*.no_alloc_save      =*/ ctx->no_alloc_save,
        /*.n_objects          =*/ 0,
        /*.objects_begin      =*/ NULL,
        /*.objects_end        =*/ NULL,
        /*.scratch            =*/ { 0, 0, NULL, },
...
void ggml_libsvc_calc_plans(struct ggml_tensor* node, struct ggml_cplan* cplan, int n_threads, int n_cur_threads)
{
    if (n_threads <= 0) {
        n_threads = GGML_DEFAULT_N_THREADS;
    }

    const int n_tasks = ggml_get_n_tasks(node, n_threads, n_cur_threads);

    const int max_tasks = MAX(1, n_tasks);
    ...
    cplan->n_threads = MIN(max_tasks, n_threads);
    if (cplan->n_threads <= 0)
        cplan->n_threads = 1;
    cplan->work_size = work_size;
    cplan->work_data = NULL;
}

void ggml_libsvc_compute_forward(struct ggml_compute_params* params, struct ggml_tensor* tensor)
{
    ggml_compute_forward(params, tensor);
}

throw_fn ggml_throw_fn_impl = NULL;

void ggml_throw(const char* message)
{
    if (ggml_throw_fn_impl)
        ggml_throw_fn_impl(message);
}

void ggml_set_throw_fn(throw_fn fun)
{
    ggml_throw_fn_impl = fun;
}
