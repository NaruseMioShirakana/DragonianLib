...
) const
{
	...
	if (_Slice.IsNotMute)
	{
		...
		const Int64 RandnShape[] = { 1, HopSize * InputTensors.Data.FrameShape[1] };
		TemplateLibrary::Vector<float> RandnInput(RandnShape[1]);
		if (ReflowSvcVersion == L"DDSPSvc")
		{
			for (auto& it : InputTensors.Data.F0)
				it = Clamp(it, F0Min, F0Max);
			for (auto& it : RandnInput)
				it = normal(gen) * _Params.DDSPNoiseScale;
			InputTensors.Tensor.emplace_back(
		...
		if (std::abs(_Params.MelFactor - 1.f) > std::numeric_limits<float>::epsilon())
		{
			auto MelOutPutData = ReflowOut[0].GetTensorMutableData<float>();
			auto MelOutputSize = ReflowOut[0].GetTensorTypeAndShapeInfo().GetElementCount();
			for (size_t i = 0; i < MelOutputSize; ++i)
				MelOutPutData[i] *= _Params.MelFactor;
		}
...
) const
{
	...
	if (EnableVolume)
	{
		...
		Volume = ExtractVolume(_PCMData);
		if (abs(int64_t(Volume.Size()) - int64_t(F0Data.Size())) > 3)
			Volume = InterpFunc(ExtractVolume(_PCMData), long(Volume.Size()), long(F0Shape[1]));
		else
	...
	const Int64 RandnShape[] = { 1, HopSize * F0Shape[1] };
	TemplateLibrary::Vector<float> RandnInput(RandnShape[1]);
	if (ReflowSvcVersion == L"DDSPSvc")
	{
		for (auto& it : RandnInput)
			it = normal(gen) * _Params.DDSPNoiseScale;
		EncoderTensors.emplace_back(
			Ort::Value::CreateTensor(
				*MemoryInfo,
				RandnInput.Data(),
				RandnInput.Size(),
...
) const
{
	...
	const Int64 RandnShape[] = { 1, HopSize * FrameShape[1] };
	TemplateLibrary::Vector<float> RandnInput(RandnShape[1]);
	if (ReflowSvcVersion == L"DDSPSvc")
	{
		for (auto& it : RandnInput)
