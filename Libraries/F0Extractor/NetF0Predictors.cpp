...
const DragonianLibOrtEnv* MelPEEnv = nullptr;
const DragonianLibOrtEnv* RmvPEEnv = nullptr;
...
inline float NetPredictorAverage(const float* begin, const float* end)
{
	const auto mp = float(end - begin);
	float sum = 0.f;
	while (begin != end)
		sum += *(begin++);
	return sum / mp;
...
DragonianLibSTL::Vector<float> RMVPEF0Extractor::ExtractF0(const DragonianLibSTL::Vector<float>& PCMData, size_t TargetLength)
{
	if (!RmvPEModel)
		return ((BaseF0Extractor*)std::make_unique<DioF0Extractor>((int)fs, (int)hop, (int)f0_bin, f0_max, f0_min).get())->ExtractF0(PCMData, TargetLength);

	const float step = float(fs) / 16000.f;
	const auto window_len = (size_t)round(step);
	const auto half_window_len = window_len / 2;
	const auto f0_size = size_t((double)PCMData.Size() / step);
	const auto pcm_idx_size = PCMData.Size() - 2;
	DragonianLibSTL::Vector pcm(f0_size, 0.f);
	auto idx = float(half_window_len + 1);
	for (size_t i = 0; i < f0_size; ++i)
	{
		const auto index = size_t(round(idx));
		if (index + half_window_len > pcm_idx_size)
		...
		if (half_window_len == 0)
			...
			pcm[i] = NetPredictorAverage(&PCMData[index - half_window_len], &PCMData[index + half_window_len]);
		idx += step;
	}
	std::vector<Ort::Value> Tensors;
	const int64_t pcm_shape[] = { 1, (int64_t)pcm.Size() };
...
DragonianLibSTL::Vector<float> MELPEF0Extractor::ExtractF0(const DragonianLibSTL::Vector<float>& PCMData, size_t TargetLength)
{
	if (!MelPEModel)
		return ((BaseF0Extractor*)std::make_unique<DioF0Extractor>((int)fs, (int)hop, (int)f0_bin, f0_max, f0_min).get())->ExtractF0(PCMData, TargetLength);
...
{
	MelPEEnv = &Env;
	MelPEModel = RefOrtCachedModel(FCPEModelPath, *MelPEEnv);
}

void LoadRMVPEModel(const wchar_t* RMVPEModelPath, const DragonianLibOrtEnv& Env)
{
	RmvPEEnv = &Env;
	RmvPEModel = RefOrtCachedModel(RMVPEModelPath, *RmvPEEnv);
}
