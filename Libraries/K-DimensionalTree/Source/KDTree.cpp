...
float KDNode::coord(const size_t& idx) const { return x.at(idx); }
KDNode::operator bool() const { return (!x.empty()); }
KDNode::operator point_t() { return x; }
KDNode::operator size_t() const { return index; }
KDNode::operator pointIndex() { return {x, index}; }
...
KDNodePtr KDTree::make_tree(const pointIndexArr::iterator& begin,  //
    ...
    auto middle = begin + static_cast<long long>(length / 2);
...
KDNodePtr KDTree::nearest_(   //
    ...
    float d = dist2(branch_pt, pt);
    float dx = branch_pt.at(level) - pt.at(level);
    float dx2 = dx * dx;
...
// default caller
KDNodePtr KDTree::nearest_(const point_t& pt) const {
    size_t level = 0;
...
point_t KDTree::nearest_point(const point_t& pt) const {
...
pointIndexArr KDTree::neighborhood_(  //
    ...
    float d = dist2(point_t(*branch), pt);
    float dx = point_t(*branch).at(level) - pt.at(level);
    float dx2 = dx * dx;
    ...
    auto nbh_s = neighborhood_(section, pt, rad, (level + 1) % dim);
    nbh.insert(nbh.end(), nbh_s.begin(), nbh_s.end());
    if (dx2 < r2) {
        auto nbh_o = neighborhood_(other, pt, rad, (level + 1) % dim);
...
pointVec KDTree::neighborhood_points(  //
    ...
    std::ranges::transform(nbh.begin(), nbh.end(), nbhp.begin(),
...
indexArr KDTree::neighborhood_indices(  //
    ...
    std::ranges::transform(nbh.begin(), nbh.end(), nbhi.begin(),
