...
Tensor<Float32, 4, Device::CPU> SingingVoiceConversionModule::ExtractUnits(
...
) const
{
	...
	auto _16KAudio = _PCMData.Continuous().Evaluate();
	if (_SamplingRate != 16000)
		_D_Dragonian_Lib_Rethrow_Block(
			_16KAudio = _16KAudio.Interpolate<Operators::InterpolateMode::Linear>(
				IDim(2),
				IScale(double(16000) / double(_SamplingRate))
			).Evaluate();
		);

	auto AudioSize = _16KAudio.ElementCount();

	auto Iter = HubertSession.find(AudioSize);
	if (Iter != HubertSession.end())
		_MyHubertSession = Iter->second;
	else
	{
		_MyHubertSession = std::make_shared<InferenceSession>();
		HubertSession[AudioSize] = _MyHubertSession;
	}

	std::vector<ITensorInfo> HubertInputTensors;
	
	HubertInputTensors.emplace_back(
		nvinfer1::Dims3(_16KAudio.Size(0), _16KAudio.Size(1), _16KAudio.Size(2)),
		HubertModel->GetInputNames()[0],
		_16KAudio.ElementCount() * sizeof(float),
	...
	if (HubertOutPutShape.d[2] != GetUnitsDim())
		_D_Dragonian_Lib_Throw_Exception("HiddenUnitKDims UnMatch");

	auto Ret = Tensor<Float32, 4, Device::CPU>::New(
		{
			1,
			HubertOutPutShape.d[0],
			HubertOutPutShape.d[1],
			HubertOutPutShape.d[2]
		}
	);

	_MyHubertSession->DeviceMemoryToHost(0, Ret.Data(), HubertSize * sizeof(float));
	return Ret;
}
...
) const
{
	...
	_D_Dragonian_Lib_Rethrow_Block(
		InferenceDatas.Units = ExtractUnits(Audio, SourceSampleRate).Evaluate();
	);
...
SliceDatas& SingingVoiceConversionModule::PreprocessNoise(
	SliceDatas& MyData,
	Int64 BatchSize,
	Int64 Channels,
	Int64 TargetNumFrames,
...

{
	if (MyData.Noise && !MyData.Noise->Null())
	{
		...
		if (NoiseDims != 192)
			_D_Dragonian_Lib_Throw_Exception("Invalid noise dims, expected: " + std::to_string(192) + ", got: " + std::to_string(NoiseDims));
	...
	{
		if (Logger)
			Logger->LogInfo(L"Noise not found, generating noise with param");
		SetRandomSeed(Seed);
		_D_Dragonian_Lib_Rethrow_Block(
			MyData.Noise = (Functional::Randn(
				IDim(BatchSize, Channels, 192, TargetNumFrames)
			) * Scale).Evaluate();
		);
	}
	return MyData;
