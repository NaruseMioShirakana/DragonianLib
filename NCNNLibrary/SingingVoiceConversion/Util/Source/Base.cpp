#include "NCNNLibrary/SingingVoiceConversion/Util/Base.hpp"
...
) const
{
	...
	_D_Dragonian_Lib_Rethrow_Block(
		InferenceDatas.Units = UnitsEncoder->Forward(Audio, SourceSampleRate, AudioMask);
	);
...
Tensor<Int32, 3, Device::CPU> SingingVoiceConversionModule::GetF0Embed(
...
)
{
	...
	auto F0Embed = Tensor<Int32, 3, Device::CPU>::New({ BatchSize, Channels, Length });
	for (SizeType i = 0; i < BatchSize; ++i)
	{
		for (SizeType j = 0; j < Channels; ++j)
		{
			...
			F0Embed.AppendTask(
				...
				(std::shared_ptr<void>) // NOLINT(performance-unnecessary-value-param)
				{
					for (SizeType k = 0; k < Length; ++k)
					{
						...
						F0EmbedData[k] = static_cast<Int32>(F0Mel);
					}
				},
				F0Cont.Buffer()
			);
...
)
{
	...
	else
	{
		...
		using UnitsSizeType = Tensor<Int32, 3, Device::CPU>;
		_D_Dragonian_Lib_Rethrow_Block(
			MyData.UnitsLength = UnitsSizeType::ConstantOf(
				{ BatchSize, Channels, 1 },
				Int32(TargetNumFrames)
			);
		);
	}
...
)
{
	...
	else
	{
		...
		).Cast<Int32>().ClampMax(Int32(UnitFrames - 1));
...
) const
{
	if (HasSpeakerEmbedding() && !HasSpeakerMixLayer())
	{
		...
		else
		{
			...
			using SpkType = Tensor<Int32, 3, Device::CPU>;
