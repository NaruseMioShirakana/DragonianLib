...
class BSRoformer(Module):
    ...
    def forward(
        ...
        d - feature dimension
        """
        batch_size = stft_repr.shape[0]     #b
        channels = stft_repr.shape[1]       #s
        frames = stft_repr.shape[2]         #t
        spec_bins = stft_repr.shape[3]      #f
        complex_bins = stft_repr.shape[4]   #c

        # [b s t f c] -> [b f s t c] stft_repr = rearrange(stft_repr,'b s f t c -> b (f s) t c')
        stft_repr = stft_repr.permute(0, 3, 1, 2, 4).reshape(batch_size, -1, frames, complex_bins)

        #x = rearrange(stft_repr, 'b f t c -> b t (f c)')
        x = stft_repr.permute(0, 2, 1, 3).reshape(batch_size, frames, -1)
        x = self.band_split(x)
        ...
        stft_repr = complex_mul(stft_repr, masks)
