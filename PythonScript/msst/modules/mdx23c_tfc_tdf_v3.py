...
class STFT:
    ...
    def __call__(self, x):
        ...
        x = torch.view_as_real(x) # [B, N, T, 2]
        x = x.permute([0, 3, 1, 2]) # [B, 2, N, T]
        x = x.reshape([*batch_dims, c, 2, -1, x.shape[-1]]).reshape([*batch_dims, c * 2, -1, x.shape[-1]]) # [B, C, 2, N, T]
        return x[..., :self.dim_f, :]
...
class TFC_TDF_net(nn.Module):
    ...
    def forward(self, x):
        ...
        b, _, f, t = x.shape
        x = x.reshape(b, self.num_target_instruments, -1, f, t).permute(1, 0, 2, 4, 3)
