...
#define RegisterTensorConstructorImp(__RegisterName, __ClassName) LibSvcSpace RegisterTensorExtractor(__RegisterName,\
	...
	{																										    \
		return new LibSvcSpace __ClassName(_srcsr, _sr, _hop, _smix, _volume,						\
...
#define RegisterSamplerImp(__RegisterName, __ClassName) LibSvcSpace RegisterSampler(__RegisterName,		 \
	...
	{																												 \
		return new LibSvcSpace __ClassName(alpha, dfn, pred, Mel_Bins, _ProgressCallback, memory);				 \
	})
...
#define RegisterReflowSamplerImp(__RegisterName, __ClassName) LibSvcSpace RegisterReflowSampler(__RegisterName,		 \
	...
	{																												 \
		return new LibSvcSpace __ClassName(velocity, Mel_Bins, _ProgressCallback, memory);				 \
	})

LibSvcHeader

LibSvcSpace SingingVoiceConversion* UnionSvcModel::GetPtr() const
{
	if (Diffusion_) return Diffusion_;
	return Reflow_;
}

DragonianLibSTL::Vector<int16_t> UnionSvcModel::InferPCMData(const DragonianLibSTL::Vector<int16_t>& _PCMData, long _SrcSamplingRate, const LibSvcSpace InferenceParams& _Params) const
{
...
DragonianLibSTL::Vector<int16_t> UnionSvcModel::SliceInference(const LibSvcSpace SingleSlice& _Slice, const LibSvcSpace InferenceParams& _Params, size_t& _Process) const
{
...
UnionSvcModel::UnionSvcModel(const LibSvcSpace Hparams& Config, const LibSvcSpace LibSvcModule::ProgressCallback& Callback, int ProviderID, int NumThread, int DeviceID)
{
	if ((Config.TensorExtractor.find(L"Diff") != std::wstring::npos) || Config.TensorExtractor.find(L"diff") != std::wstring::npos)
		Diffusion_ = new LibSvcSpace DiffusionSvc(
			Config, Callback,
			LibSvcSpace LibSvcModule::ExecutionProviders(ProviderID),
			DeviceID, NumThread
		);
	else
		Reflow_ = new LibSvcSpace ReflowSvc(
			Config, Callback,
			LibSvcSpace LibSvcModule::ExecutionProviders(ProviderID),
			DeviceID, NumThread
		);
